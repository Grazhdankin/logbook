...
public class WeatherLogResource {
    ...
    public ResponseEntity<WeatherLogDto> findOne(@PathVariable Long id) {
    ...
    public List<WeatherLogDto> findByGeneralLogId(@PathVariable Long id) {
        return endpoint.findByGeneralLogId(id);
    }

    @Secured({"ROLE_USER", "ROLE_CREATOR", "ROLE_EDITOR", "ROLE_ADMIN"})
    @GetMapping(path = "/revisions/{id}", produces = MediaType.APPLICATION_JSON_VALUE)
    public List<WeatherLogDto> findAllRevisions(@PathVariable Long id) {
        return endpoint.findAllRevisions(id);
    }

    @Secured({"ROLE_CREATOR", "ROLE_EDITOR", "ROLE_ADMIN"})
    @PostMapping(consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public WeatherLogDto save(@Validated(Group.Create.class) @RequestBody WeatherLogDto dto) {
        return endpoint.save(dto);
    }

    @Secured({"ROLE_EDITOR", "ROLE_ADMIN"})
    @PutMapping(consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
    public WeatherLogDto update(@Validated(Group.Update.class) @RequestBody WeatherLogDto dto) {
        return endpoint.save(dto);
    }
