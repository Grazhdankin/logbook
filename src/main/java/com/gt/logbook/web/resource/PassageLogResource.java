...
public class PassageLogResource {
    ...
    public Mono<ResponseEntity<PassageLogDto>> findOne(@PathVariable Long id) {
        return Mono.fromCallable(() -> endpoint.findOne(id)).map(ResponseEntity::of);
    }

    @PostMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<PassageLogDto> save(@Validated(Group.Create.class) @RequestBody PassageLogDto dto) {
        return Mono.fromCallable(() -> endpoint.save(dto));
    }

    @PutMapping(produces = MediaType.APPLICATION_JSON_VALUE)
    public Mono<PassageLogDto> update(@Validated(Group.Update.class) @RequestBody PassageLogDto dto) {
        return Mono.fromCallable(() -> endpoint.save(dto));
    }
